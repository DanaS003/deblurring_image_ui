...
from natten import NeighborhoodAttention2D # This import is from the provided notebook
...
class TransBlock(nn.Module):
    def __init__(self, dim, num_heads, kernel, dilation, ffn_expansion_factor, bias, sa=True):
        ...
        if sa == True:
            ...
            # Revert to passing 'dim' directly as per the original notebook code
            self.na2d = NeighborhoodAttention2D(dim, kernel_size=kernel, \
...
class Embeddings_output(nn.Module):
    def __init__(self, dim, num_blocks, kernel, heads, bias):
        ...
        self.de_trans_level3 = nn.Sequential(*[
            item for sublist in 
            [[TransBlock(dim*2**2, heads[2], kernel, 1, 1, bias=bias, sa=True),
             TransBlock(dim*2**2, heads[2], kernel, 9, 1, bias=bias, sa=True)] for i in range(num_blocks[2]//2)] for item in sublist
        ])
        ...
        self.de_trans_level2 = nn.Sequential(*[
            item for sublist in 
            [[TransBlock(dim*2, heads[1], kernel, 1, 1, bias=bias, sa=True),
             TransBlock(dim*2, heads[1], kernel, 18, 1, bias=bias, sa=True)] for i in range(num_blocks[1]//2)] for item in sublist
        ])
        ...
        self.de_trans_level1 = nn.Sequential(*[
            item for sublist in 
            [[TransBlock(dim, heads[0], kernel, 1, 1, bias=bias, sa=True),
             TransBlock(dim, heads[0], kernel, 36, 1, bias=bias, sa=True)] for i in range(num_blocks[0]//2)] for item in sublist
        ])
